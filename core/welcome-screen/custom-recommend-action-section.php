 ...
 * @access public
 */
class Allegiant_Customize_Section_Recommend extends WP_Customize_Section {
	/**
	 ...
	 * @var    string
	 */
	public $type = 'allegiant-recomended-section';
	/**
	...
	public function json() {
		...
		foreach ( $allegiant_required_actions as $key => $allegiant_required_action ) {
			if ( @$allegiant_show_required_actions[ $allegiant_required_action['id'] ] === false ) {
				continue;
			}
			if ( $allegiant_required_action['check'] ) {
				continue;
			}

			$allegiant_required_action['index'] = $key + 1;

			if ( isset($allegiant_required_action['plugin_slug']) ) {
				$active = $this->check_active( $allegiant_required_action['plugin_slug'] );
				$allegiant_required_action['url']    = $this->create_action_link( $active['needs'], $allegiant_required_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$allegiant_required_action['class'] = 'active';
				}else{
					$allegiant_required_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$allegiant_required_action['button_class'] = 'install-now button';
						$allegiant_required_action['button_label'] = __( 'Install', 'allegiant' );
						break;
					case 'activate':
						$allegiant_required_action['button_class'] = 'activate-now button button-primary';
						$allegiant_required_action['button_label'] = __( 'Activate', 'allegiant' );
						break;
					case 'deactivate':
						$allegiant_required_action['button_class'] = 'deactivate-now button';
						$allegiant_required_action['button_label'] = __( 'Deactivate', 'allegiant' );
						break;
				}

			}
			$formatted_array[] = $allegiant_required_action;
		}
		...
		foreach ( $allegiant_recommended_plugins as $slug => $plugin_opt ) {
			...
			$allegiant_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$allegiant_recommended_plugin['class'] = 'active';
			}else{
				$allegiant_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$allegiant_recommended_plugin['button_class'] = 'install-now button';
					$allegiant_recommended_plugin['button_label'] = __( 'Install', 'allegiant' );
					break;
				case 'activate':
					$allegiant_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$allegiant_recommended_plugin['button_label'] = __( 'Activate', 'allegiant' );
					break;
				case 'deactivate':
					$allegiant_recommended_plugin['button_class'] = 'deactivate-now button';
					$allegiant_recommended_plugin['button_label'] = __( 'Deactivate', 'allegiant' );
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$allegiant_recommended_plugin['id'] = $slug;
			$allegiant_recommended_plugin['plugin_slug'] = $slug;
			$allegiant_recommended_plugin['description'] = $info->short_description;
			$allegiant_recommended_plugin['title'] = $allegiant_recommended_plugin['button_label'].': '.$info->name;

			$customize_plugins[] = $allegiant_recommended_plugin;
		...
		$json['total_actions'] = count($allegiant_required_actions);
		$json['social_text'] = $this->social_text;
		$json['plugin_text'] = $this->plugin_text;
		$json['facebook'] = $this->facebook;
		$json['facebook_text'] = __( 'Facebook', 'allegiant' );
		$json['twitter'] = $this->twitter;
		$json['twitter_text'] = __( 'Twitter', 'allegiant' );
		$json['wp_review'] = $this->wp_review;
		$json['wp_review_text'] = __( 'Review this theme on w.org', 'allegiant' );
		if ( $this->wp_review ) {
	...
	protected function render_template() { ?>
		<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
			...
			<h3 class="accordion-section-title">
				...
				<# if( data.required_actions.length > 0 ){ #>
					<span class="allegiant-actions-count">
			...
			<div class="recomended-actions_container" id="plugin-filter">
						...
						<div class="epsilon-recommeded-actions-container epsilon-required-actions" data-index="{{ data.required_actions[action].index }}">
							<# if( !data.required_actions[action].check ){ #>
								<div class="epsilon-recommeded-actions">
									...
									<span data-action="dismiss" class="dashicons dashicons-visibility allegiant-dismiss-required-action" id="{{ data.required_actions[action].id }}"></span>
						...
						<div class="epsilon-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
							<# if( !data.recommended_plugins[action].check ){ #>
								<div class="epsilon-recommeded-actions">
									...
									<span data-action="dismiss" class="dashicons dashicons-visibility allegiant-recommended-plugin-button" id="{{ data.recommended_plugins[action].id }}"></span>
				...
				<# if( data.required_actions.length == 0 && data.recommended_plugins.length == 0 ){ #>
					<p class="succes">
						...
						<# } #>
						<# if( data.wp_review ){ #>
							<a href="https://wordpress.org/support/theme/{{ data.theme_slug }}/reviews/#new-post" class="button button-primary allegiant-wordpress" target="_blank"><span class="dashicons dashicons-wordpress"></span>{{ data.wp_review_text }}</a>
						<# } #>
					</p>
				<# }else{ #>
					<p class="succes hide">
