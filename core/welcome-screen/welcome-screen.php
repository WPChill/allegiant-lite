...
class Allegiant_Welcome {
	...
	public function __construct() {
		...
		add_action( 'customize_controls_enqueue_scripts', array( $this, 'allegiant_welcome_scripts_for_customizer' ) );
		...
		add_action( 'wp_ajax_nopriv_allegiant_dismiss_required_action', array(
			$this,
			'allegiant_dismiss_required_action_callback'
		) );
	 ...
	 * @since  1.8.2.4
	 */
	public function allegiant_welcome_scripts_for_customizer() {
		...
		wp_localize_script( 'cpotheme-welcome-screen-customizer-js', 'allegiantWelcomeScreenCustomizerObject', array(
			'nr_actions_required' => $this->count_actions(),
			...
			'themeinfo'           => __( 'View Theme Info', 'allegiant' ),
		) );
	}
	...
	public function allegiant_dismiss_required_action_callback() {
		...
		$allegiant_dismiss_id = ( isset( $_GET['dismiss_id'] ) ) ? $_GET['dismiss_id'] : 0;

		echo $allegiant_dismiss_id; /* this is needed and it's the id of the dismissable required action */

		if ( ! empty( $allegiant_dismiss_id ) ):
			...
			if ( get_option( 'allegiant_show_required_actions' ) ):
				...
				$allegiant_show_required_actions[ $allegiant_dismiss_id ] = false;
			...
			/* create the new option,with false for the specified id */
				...
				if ( ! empty( $allegiant_required_actions ) ):
					...
					foreach ( $allegiant_required_actions as $allegiant_required_action ):
						...
						if ( $allegiant_required_action['id'] == $allegiant_dismiss_id ):
		...
		<div class="wrap about-wrap epsilon-wrap">
			...
			<h2 class="nav-tab-wrapper wp-clearfix">
				...
				<a href="<?php echo admin_url( 'themes.php?page=cpotheme-welcome&tab=changelog' ); ?>"
				   class="nav-tab <?php echo $active_tab == 'changelog' ? 'nav-tab-active' : ''; ?> "><?php echo esc_html__( 'Changelog', 'allegiant' ); ?></a>
			</h2>
